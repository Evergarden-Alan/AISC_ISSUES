...
// 仓库根索引 索引.md 构建（v0.1.2）：反馈/需求两表、序号按时间升序、单元格清洗
...
test("两表分离：反馈与需求各自成表", () => {
  ...
  assert.ok(md.includes("| 1 | 导出报表时软件闪退 | 处理中 |"));
  assert.ok(md.includes("| 1 | 希望能批量导出 | 已收到 |")); // 两表独立编号
});
...
test("序号按 createdAt 升序；状态双映射按 category 取词", () => {
  ...
  assert.ok(
    issuePart.indexOf("| 1 | 问题一 | 已解决 |") <
      issuePart.indexOf("| 2 | 问题二 | 暂不处理 |")
  );
  assert.ok(
    featPart.indexOf("| 1 | 功能A | 已收到 |") <
      featPart.indexOf("| 2 | 功能B | 已上线 |")
  );
});
...
test("单元格清洗：竖线/换行不破坏表格；空表有占位行", () => {
  ...
  assert.ok(md.includes("| 1 | 标题／带竖线 换行 | 已收到 |"));
