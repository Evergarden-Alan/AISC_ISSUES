...
- 追加清理（2026-09-23）：数据仓库 v0.1.0 旧布局 feedback/ 目录整树移除（Git Data API，内容仅为 3 张失效暂存截图）；cron 旧路径扫描代码随之删除，布局守卫测试收紧为零容忍。
...
11. **路径迁移不要用字符串批量替换**——模板字面量的间接拼接（`` `feedback/assets/${ref}` ``）不会被字面量替换命中。目录/路径迁移应先收口为常量（如 PENDING_BASE），并配一个「守卫测试」扫描源码禁止旧字面量。v0.1.2 当天因此连出两起故障（图片过期复发、详情页 404），守卫测试（落盘 + 渲染层）是必须的。
...
13. **写入后必须主动失效读缓存**（Next 16：fetch 打 tags + `revalidateTag(tag, "max")`，注意新签名要求第二个参数），否则「提交成功但页面 5 分钟看不到」；客户端导航还要注意路由缓存——期望看到最新数据的跳转用整页加载。
...
- 每个用户可见 bug 修复都补单测/e2e 回归，测试随版本只增不减（55 → 102）。
